...
## ROUND 3 — corpus sanity-sweep fixes, FIXED & VERIFIED (2026-07-05)
Source: full-corpus data sanity sweep (`benefits-hub-handoff/06-audit/DATA_SANITY_SWEEP.md`),
all 8,081 plans scanned, close calls verified against raw PBP .txt files. All plans
regenerated; batch card compiler re-run: 8,081/8,081, 0 build failures, 0 single-surface leaks.

| # | Benefit (JSON key) | Symptom | Root cause → Fix | Verified |
|---|---|---|---|---|
| R3-1 | Hearing aids (`hearing_aids.max_plan_benefit_per_ear`) | "$1"/"$2"/"$3" per-ear maximums on 2,081 plans | `pbp_b18b_maxplan_perear` is a "Select Coverage" ENUM (1=Per ear, 2=One single ear, 3=Both ears combined) qualifying `maxplan_amt`, not a dollar; it was run through `to_dollar`. Replaced with `max_plan_benefit_scope` string attached to the real `max_plan_benefit`; bogus dollar field removed; hearing base template updated to render "…{scope}" | H0028_007_0 now `$2000` + `per ear` (raw: amt=2000, perear=1); residual bogus per-ear dollars corpus-wide: **0** |
| R3-2 | False-coverage stubs (`transportation`, `meals`, `otc_items`, `dental_*`, `hearing_aids`, `telehealth`, `additional_preventive`) | Label-only benefit dicts emitted for benefits the raw filing says are NOT offered (up to 5,268 plans/key) | Four gate defects: (a) explicit `bendesc_yn = 2` did not stop `extract_simple_benefit`; (b) alternate offered-flags `bendesc_service`/`bendesc_otc` are 1/2 codes but were only checked for non-emptiness ("2" passed); (c) transportation fell back to the ungated simple extractor when the supplemental extractor correctly returned None for yn=2; (d) no substance check at assembly. Fixes: explicit-No returns None in both extractors; "2" rejected on alternate flags; fallback gated; central `SUPPLEMENTAL_STUB_KEYS` label-stub filter in `build_plan_detail` (PACE plans exempt — label-only is their normal filing; mandatory benefits like urgent care/ER exempt — covered even with no cost detail filed) | H0028_014_0 no longer emits `transportation` (raw yn=2); PACE H0016_001_0 keeps its 37 label-only benefits; residual non-PACE supplemental stubs corpus-wide: **0**. Compiled-card deltas match sweep counts exactly: meals 6,986→3,321, otc 5,084→2,326 |

| R3-3 | Meals (`meals.meal_types`) | Card showed a generic "Meals" with no indication of when the benefit applies | `pbp_b13c_meal_type_chk` (3-char flag: 1=homebound, 2=post-discharge, 3=chronic illness) was never extracted. Now emitted as a member-facing `meal_types` string; meals gate also hardened to `yn()==Yes`. | H5425_118_0 → "when a health condition keeps you at home or for a chronic illness" (101); H7617_076_0 → "after surgery or a hospital stay" (010). Corpus: 010 post-discharge dominant (2,670), but 011/111/101/etc. confirm generic labeling would misstate ~660 plans |

Known source gap (not fixable from PBP): 367 standalone PDPs have no Part D premium anywhere
in the PBP files — filling it requires a CMS Landscape-file join (future enhancement).
...
| R2-0 | Fitness benefit (`fitness`) — fixed 2026-07-01, pre-Round-2 | B14c fitness data (5,115 plans) extracted under the label "Mental Health Counseling" and buried in `additional_preventive.sub_types`; no top-level fitness benefit existed | Column suffix `_mhc` (legacy "membership health club") is service category 14c4 = Fitness Benefit per the 2026 dictionary, not mental health. Removed the mislabeled sub-type; new top-level extractor emits `benefit_type`, `fitness_types` (typ_mhc 3-char flags: Activity Tracker / Physical Fitness / Memory Fitness), copay, coinsurance, maxplan (+period), maxenr | 5,115 plans emit `fitness` (= 5,091 amo_mhc=2 + 24 =3 raw rows); SCAN H5425-123 "Physical Fitness / Mandatory" matches its SB ($0 via One Pass) |
